...
First, we used `str()` function to display the structure of the data, identifying both categorical and quantitative variables. We used the `sum()` function to check for any missing values in the data. Noticing some repeated rows, we applied the `distinct()` function to extract unique rows from the raw data. To identify common names across different ethnic groups, we merged two datasets using the `inner_join()` function, allowing us to detect any overlapping names. We mainly used some functions like `group_by`, `filter`, and `summarize` from the dplyr package to form sub datasets. Additionally, we utilized `str_count` to calculate the number of letters in each name, and `mutate` to add a new column displaying this count. We then used `group_by` to calculate the average name length by gender and for each ethnic group. For data visualization, the `ggplot2` package was our primary tool. Lastly, we created a shiny app in order that we can search or filter by a category to explore the data. 
