...
#| fig-height: 4 
#| out-width: 80%  
#| fig-cap: "This figure shows unisex names"
#| echo: false

library(tidyr)

ggplot(data = inner_COM, aes(x = Child.s.First.Name, y = value, fill = variable)) + 
  geom_bar(stat = "identity", position = position_dodge()) +  
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +  
  labs(title = "Unisex Names",
        subtitle = "top 10", 
       x = "Name",
       y = "Frequency",
       fill = "Gender")+
...
By merging these datasets, for each ethnic group, the more unisex names are used in WNH. 
