...
After reading data files, we used `str()` function to display the structure of the data, identifying both categorical and quantitative variables. We used the `sum(is.na())` function to check for any missing values in the raw data. Noticing some repeated rows, we applied the `distinct()` function to extract unique rows from the raw data. When we subset the dataset regarding the different genders, different ethnic groups, we mainly used `dplyr` functions `group_by`, `filter`, `summarise`, `arrange` and `mutate`[@dplyr-package] to achieve our goal. For example, to generate a dataset consisting popular babies' names for each gender, we used `filter` function select gender of male first, then used `group_by` to get the dataset consisting of all males names, and used  `summarize` to count the frequencies of each male names occurance and use `arrange` function to arrange the frequency and `head` function to extract the top 10 popular males babies names. For the top 10 popular females names, we only need to change the male to femal when we use `filter` function. 

To identify common names across different ethnic groups, we merged two datasets using the `inner_join()` function, allowing us to detect any overlapping names.  like `group_by`, `filter`, and `summarize` from the dplyr package to form sub datasets. Additionally, we utilized `str_count` to calculate the number of letters in each name, and `mutate` to add a new column displaying this count. We then used `group_by` to calculate the average name length by gender and for each ethnic group. 

For data visualization, the `ggplot2` package was our primary tool. 

Lastly, we created a shiny app in order that we can search or filter by a category to explore the data.
